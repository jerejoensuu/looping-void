...
# Start considering landings only after 3 ticks in forgiven fall.

# 1. Soft landing: water / lava / cobweb etc → end, but NO custom damage
execute if score @s av_timer matches 3.. if predicate aether_void:is_soft_landing run function aether_void:end_void_fall_nodamage

# 2. Hard landing: on solid ground → end with custom landing damage
execute if score @s av_timer matches 3.. if predicate aether_void:is_landed run function aether_void:end_void_fall

# 3. Failsafe: if we have been in state too long, end anyway (with damage)
execute if score @s av_timer >= #maxTicks av_cfg run function aether_void:end_void_fall
